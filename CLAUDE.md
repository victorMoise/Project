...
9. ~~Login Keycloak (Authorization Code + PKCE) în `mobile/`~~ — făcut (2026-09-26). Scaffold: `create-expo-app`, template `blank-typescript`, SDK 57, migrat la Expo Router (`src/app/`, `main: expo-router/entry`, `scheme: mobile`, alias `@/*` → `./src/*`). Librărie de auth: **`expo-auth-session`**, nu `react-native-app-auth` — motiv concret: sub capotă folosește tot `ASWebAuthenticationSession`/Custom Tabs prin `expo-web-browser` (aceeași securitate ca AppAuth), dar rămâne utilizabil în Expo Go, fără development build de la prima zi. Client Keycloak nou, `mobile-app` (realm `project`) — public, doar Standard Flow, PKCE method `S256`, `redirectUris`: `mobile://redirect` (development build/standalone) și `exp://*` (Expo Go, dev-only — wildcard, nu pentru producție), separat de `dev-testing`. Flux implementat: `AuthProvider` (`src/context/auth-context.tsx`) — `useAutoDiscovery` + `useAuthRequest` + `exchangeCodeAsync`, sesiune persistată prin `useStorageState` (`expo-secure-store`, pattern-ul oficial Expo Router pentru auth), refresh la cerere prin `getAccessToken()` (`refreshAsync`, cu leeway de 30s). Rutare: `Stack.Protected` în `src/app/_layout.tsx` (`(app)` dacă `session`, altfel `sign-in`), `SplashScreenController` ține splash-ul până se termină citirea din SecureStore. Ecranul principal (`(app)/index.tsx`) apelează Gateway (`/collections-service/api/items`) cu access token-ul, ca verificare end-to-end. Config prin `EXPO_PUBLIC_*` în `mobile/.env` (gitignored, `.env.example` checked in): `EXPO_PUBLIC_KEYCLOAK_ISSUER`, `EXPO_PUBLIC_KEYCLOAK_CLIENT_ID`, `EXPO_PUBLIC_GATEWAY_URL`. Netestat încă pe device/simulator real (doar typecheck + lint + `expo-doctor` 21/21 + bundling smoke test cu `expo export --platform ios`) — de verificat live la următoarea sesiune de lucru pe `mobile/`.
